...
class NASDistillationIntegration:
    ...
    def __init__(self,
                 ...
                 alpha: float = 0.7,
                 distillation_type: str = 'multi_teacher',
        ...
        Args:
            ...
            temperature: 蒸馏温度
            alpha: 蒸馏损失权重
            distillation_type: 蒸馏类型 ('multi_teacher', 'progressive', 'single_teacher')
            save_dir: 模型保存目录
        """
    ...
    def initialize_distillation(self) -> None:
        ...
        logging.info(f"初始化知识蒸馏组件，蒸馏类型: {self.distillation_type}")
        
        # 根据蒸馏类型创建相应的蒸馏模型
        if self.distillation_type == 'multi_teacher':
            # 多教师蒸馏
        ...
        elif self.distillation_type == 'progressive':
            # 渐进式知识蒸馏
            from knowledge_distillation import ProgressiveKnowledgeDistillation
            self.distillation_model = ProgressiveKnowledgeDistillation(
                teacher_models=list(self.teacher_ensemble.values()),
                student_model=self.student_model,
                device=self.device,
                temperature=self.temperature,
                alpha=self.alpha
            )
        elif self.distillation_type == 'single_teacher':
            # 单教师蒸馏（使用第一个教师模型）
            first_teacher = list(self.teacher_ensemble.values())[0]
            self.distillation_model = MultiTeacherDistillation(
                teachers={'teacher_0': first_teacher},
                student=self.student_model,
                temperature=self.temperature,
                alpha=self.alpha,
                freeze_teachers=True
            ...
            raise ValueError(f"不支持的蒸馏类型: {self.distillation_type}. "
                           f"支持的类型: 'multi_teacher', 'progressive', 'single_teacher'")
        
        self.distillation_model = self.distillation_model.to(self.device)
        logging.info(f"知识蒸馏组件初始化完成，使用{self.distillation_type}蒸馏")
