    ...
    def _create_self_attention(self, channels: int) -> nn.Module:
        class SelfAttention3D(nn.Module):
            def __init__(self, channels):
                ...
                self.channels = channels
                # 大幅减少注意力头数以节省内存
                self.num_heads = min(2, max(1, channels // 16))  # 最多2个头，每头至少16个通道
                # 减少嵌入维度以节省内存
                self.embed_dim = min(channels, 64)  # 限制最大嵌入维度为64
                ...
                if channels != self.embed_dim:
                    self.input_proj = nn.Linear(channels, self.embed_dim)
                    self.output_proj = nn.Linear(self.embed_dim, channels)
                else:
            ...
            def forward(self, x):
                ...
                # 大幅减少空间维度以节省内存
                # 只在最小的空间维度上应用注意力，并进一步下采样
                spatial_size = min(H, W)
                if spatial_size > 8:
                    # 如果空间维度太大，先进行下采样
                    downsample_factor = spatial_size // 8
                    x_downsampled = F.avg_pool3d(x, kernel_size=(1, downsample_factor, downsample_factor), 
                                                stride=(1, downsample_factor, downsample_factor))
                    _, _, D_down, H_down, W_down = x_downsampled.shape
                    x_work = x_downsampled
                else:
                    x_work = x
                    H_down, W_down = H, W
                
                # 只处理前几个深度切片以节省内存
                max_depth_slices = min(D, 4)
                x_work = x_work[:, :, :max_depth_slices, :, :]
                
                # 重塑为序列格式: (B, seq_len, embed_dim)
                x_reshaped = x_work.view(B, C, -1).transpose(1, 2)  # (B, max_depth_slices*H_down*W_down, C)
                
                # 应用输入投影（如果需要）
                if self.input_proj is not None:
                    x_reshaped = self.input_proj(x_reshaped)
                ...
                # 重塑回空间格式
                attn_out = attn_out.transpose(1, 2).view(B, C, max_depth_slices, H_down, W_down)
                
                # 如果进行了下采样，需要上采样回原始尺寸
                if spatial_size > 8:
                    attn_out = F.interpolate(attn_out, size=(max_depth_slices, H, W), mode='trilinear', align_corners=False)
                
                # 如果深度维度被裁剪，需要扩展回原始深度
                if max_depth_slices < D:
                    # 复制最后一个切片来填充剩余的深度
                    last_slice = attn_out[:, :, -1:, :, :]
                    padding_slices = last_slice.repeat(1, 1, D - max_depth_slices, 1, 1)
                    attn_out = torch.cat([attn_out, padding_slices], dim=2)
                
                # 应用非常小的残差连接权重
                return x + 0.01 * attn_out  # 进一步减小注意力的影响
...
class SuperNet(nn.Module):
    ...
    def __init__(self, 
                 in_channels: int = 4,
                 num_classes: int = 4,
                 base_channels: int = 6,  # 进一步减少默认base_channels以节省更多内存
                 num_layers: int = 2,  # 进一步减少默认层数
        ...
        for i in range(num_layers):
            ...
            if i == 0:
                ...
                out_channels = base_channels * 2  # 第二层翻倍
            else:
                out_channels = base_channels * 2  # 后续层保持2倍
...
class ProgressiveNAS:
    ...
    def create_supernet(self, num_layers: int) -> SuperNet:
        ...
        """
        return SuperNet(
            base_channels=6,  # 大幅减少base_channels以节省内存
            num_layers=min(num_layers, 2),  # 限制最大层数
