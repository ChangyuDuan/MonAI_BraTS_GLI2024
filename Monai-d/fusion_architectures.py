        ...
        for i, model in enumerate(self.base_models):
            ...
            for level, target_channels in enumerate(self.fusion_channels):
                ...
                if not hasattr(self, f'channel_adapters_{i}'):
                    # 动态创建通道适配器，使用动态的输出通道数
                    model_out_channels = self.num_classes if hasattr(self, 'num_classes') else 4
                    setattr(self, f'channel_adapters_{i}', 
                           nn.ModuleList([
                               nn.Conv3d(model_out_channels, ch, 1).to(x.device) for ch in self.fusion_channels
                           ]))
