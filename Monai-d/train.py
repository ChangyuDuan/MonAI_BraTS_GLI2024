...
from MSMultiSpineLoader import MSMultiSpineDatasetLoader, create_optimized_training_config, ImbalancedLoss
from model import BasicModelBank, ModelFactory, create_optimizer, create_scheduler, SpecializedModelFactory
...
class ModelTrainer:
    ...
    def _setup_data(self):
        ...
        设置数据加载器
        """
        dataset_type = self.config.get('dataset_type', 'BraTS')
        print(f"设置数据加载器... (数据集类型: {dataset_type})")
        
        # 强制设置num_workers=0以避免Windows多进程问题
        import platform
        forced_num_workers = 0  # 在Windows上强制设置为0
        if platform.system() == 'Windows':
            print(f"[Windows兼容性] 强制设置num_workers=0以避免DataLoader worker问题")
        
        # 根据数据集类型选择相应的数据加载器
        if dataset_type == 'MS_MultiSpine':
            # 检查是否启用优化策略
            use_optimization = self.config.get('use_optimization', True)
            
            if use_optimization:
                print("[优化策略] 启用MS_MultiSpine数据集优化策略")
                # 创建优化配置
                ...
                print(f"[优化策略] 自适应裁剪: {optimized_config['strategies']['adaptive_crop']}")
                print(f"[优化策略] 渐进式增强: {optimized_config['strategies']['progressive_augmentation']}")
                print(f"[优化策略] 不平衡损失: {optimized_config['strategies']['imbalanced_loss']}")
            else:
                print("[标准模式] 使用标准MS_MultiSpine数据加载器")
                data_loader = MSMultiSpineDatasetLoader(
                    data_dir=self.config['data_dir'],
                    cache_rate=self.config.get('cache_rate', 0.1),
                    num_workers=forced_num_workers,  # 强制设置为0避免多进程问题
                    seed=self.config.get('seed', 42)
                )
        else:  # BraTS
        ...
        # 针对NAS训练优化batch_size以减少内存使用
        model_category = self.config.get('model_category', 'basic')
        model_type = self.config.get('model_type', 'fusion')
        
        # 如果是NAS相关训练，使用更小的batch_size
        if model_category == 'advanced' and ('nas' in model_type.lower()):
            default_batch_size = 1  # NAS训练使用batch_size=1以节省内存
            print(f"[内存优化] 检测到NAS训练，将batch_size设置为1以减少CUDA内存使用")
        else:
            default_batch_size = 2  # 其他训练使用默认batch_size=2
    ...
    def _setup_model(self):
        ...
        if model_category == 'advanced':
            print(f"\n[模型类型] 复合架构模型: {self.config.get('model_type', 'fusion').upper()}")
            
            # 使用ModelFactory创建复合架构模型
            ...
            # 为复合架构模型创建完整评估指标
            temp_model_creator = BasicModelBank(
                device=self.device,
                dataset_type=self.config.get('dataset_type', 'BraTS')
            )
            ...
            print(f"[复合架构模型] 类型: {self.config.get('model_type', 'fusion')}")
            
            # 对于复合架构模型，检查是否有parameters方法
            if hasattr(self.model, 'parameters') and callable(getattr(self.model, 'parameters')):
                try:
                    param_count = sum(p.numel() for p in self.model.parameters())
                    print(f"[参数统计] 模型参数数量: {param_count:,}")
                except Exception as e:
                    print(f"[参数统计] 无法统计参数数量: {e}")
            else:
                print(f"[参数统计] 复合架构模型，参数统计将在训练阶段进行")
            ...
            # 检查是否使用优化损失函数（仅对MS_MultiSpine数据集）
            dataset_type = self.config.get('dataset_type', 'BraTS')
            use_optimization = self.config.get('use_optimization', True)
            
            if dataset_type == 'MS_MultiSpine' and use_optimization and hasattr(self, 'optimized_config'):
                print("[优化策略] 使用ImbalancedLoss优化损失函数")
                self.loss_function = self.optimized_config['loss_function']
            else:
        ...
        if hasattr(self, 'is_advanced') and self.is_advanced:
            # 复合架构模型训练模式
            loss_info = {
                'strategy': 'Composite Architecture Model Loss',
                'type': self.config.get('model_type', 'fusion'),
                'description': f'Composite {self.config.get("model_type", "fusion")} model with specialized loss function'
            }
        else:
            # 单模型训练模式
        ...
        # 根据数据集类型设置后处理变换
        dataset_type = self.config.get('dataset_type', 'BraTS')
        num_classes = 6 if dataset_type == 'MS_MultiSpine' else 4
    ...
    def _setup_training(self):
        ...
        if hasattr(self, 'is_advanced') and self.is_advanced:
            ...
            elif model_type == 'nas_distillation':
                # NAS-蒸馏集成模型不需要优化器，因为它有自己的训练流程
                self.optimizer = None
            ...
            if hasattr(self, 'arch_optimizer'):
                ...
                # 标准单模型优化器
                # 检查是否使用优化训练参数（仅对MS_MultiSpine数据集）
                dataset_type = self.config.get('dataset_type', 'BraTS')
                use_optimization = self.config.get('use_optimization', True)
                
                if dataset_type == 'MS_MultiSpine' and use_optimization and hasattr(self, 'optimized_config'):
                    print("[优化策略] 使用优化训练参数")
                    training_recommendations = self.optimized_config['training_recommendations']
                    
                    self.optimizer = create_optimizer(
                        self.model,
                        optimizer_name=training_recommendations.get('optimizer', 'adamw'),
                        learning_rate=training_recommendations.get('learning_rate', 1e-4),
                        weight_decay=training_recommendations.get('weight_decay', 1e-5)
                    )
                    
                    print(f"[优化策略] 优化器: {training_recommendations.get('optimizer', 'adamw')}")
                    print(f"[优化策略] 学习率: {training_recommendations.get('learning_rate', 1e-4)}")
                    print(f"[优化策略] 权重衰减: {training_recommendations.get('weight_decay', 1e-5)}")
                    print(f"[优化策略] 推荐批次大小: {training_recommendations.get('batch_size', 2)}")
                    print(f"[优化策略] 混合精度: {training_recommendations.get('mixed_precision', True)}")
                else:
                    self.optimizer = create_optimizer(
                        self.model,
                        optimizer_name=self.config.get('optimizer', 'adamw'),
    ...
    def _setup_output_directories(self):
        ...
        if model_category == 'advanced':
            # 复合架构模型：./outputs/models/{model_type}_model/
            model_type = self.config.get('model_type', 'fusion')
    ...
    def train(self):
        ...
        # 检查是否是NAS-蒸馏集成模型，需要特殊处理
        if (hasattr(self, 'is_advanced') and self.is_advanced and 
            self.config.get('model_type') == 'nas_distillation'):
            print("[特殊训练模式] NAS-蒸馏集成模型，使用专用训练流程")
            self._train_nas_distillation()
            return
    ...
    def _train_nas_distillation(self):
        """
        NAS-蒸馏集成模型的专用训练流程
        """
        print("执行NAS-蒸馏集成训练流程...")
        
        try:
            # 直接调用NASDistillationIntegration的完整训练流程
            results = self.model.full_training_pipeline(
                self.train_loader, 
                self.val_loader
            )
            
            print("NAS-蒸馏集成训练完成！")
            print(f"NAS搜索结果: 最优验证损失 = {results['nas_search']['val_loss']:.4f}")
            print(f"知识蒸馏结果: 最优验证损失 = {results['distillation']['best_val_loss']:.4f}")
            
            # 保存训练结果到输出目录
            import json
            results_path = self.output_dir / 'nas_distillation_results.json'
            ...
            print(f"模型检查点保存在: {self.model.save_dir}")
            
        except Exception as e:
            print(f"[错误] NAS-蒸馏集成训练失败: {e}")
            import traceback
            traceback.print_exc()
            raise
