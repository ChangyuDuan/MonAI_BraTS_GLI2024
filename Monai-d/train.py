...
class ModelTrainer:
        ...
        try:
            ...
            with torch.no_grad():
                for batch_data in self.val_loader:
                    ...
                    if hasattr(self, 'model_creator'):
                        # 获取推理参数
                        roi_size = self.config.get('roi_size', (96, 96, 96))
                        sw_batch_size = self.config.get('sw_batch_size', 4)
                        overlap = self.config.get('overlap', 0.5)
                        
                        outputs = self.model_creator.sliding_window_inference(
                            inputs, 
                            roi_size=roi_size,
    ...
    def evaluate(self, test_loader=None):
        ...
        Args:
            test_loader: 测试数据加载器，如果为None则使用验证集
        ...
        # 使用测试集或验证集
        eval_loader = test_loader if test_loader is not None else self.val_loader
        ...
        print(f"使用推理参数: roi_size={roi_size}, sw_batch_size={sw_batch_size}, overlap={overlap}")
        
        with torch.no_grad():
            for batch_data in tqdm(eval_loader, desc="评估中"):
                inputs = batch_data['image'].to(self.device)
                labels = batch_data['label'].to(self.device)
        ...
        print(f"  样本数量: {len(eval_loader.dataset)}")
