...
def auto_adjust_parameters(device: torch.device, args):
    ...
    根据设备自动调节训练参数
    """
    # 基础参数 - 只有当用户没有传入参数时才使用默认值
    base_batch_size = args.batch_size if args.batch_size is not None else 2
    base_epochs = args.epochs if args.epochs is not None else 500
    base_lr = args.learning_rate if args.learning_rate is not None else 1e-4
    
    if device.type == 'cuda':
        # GPU设备，使用优化参数
        gpu_memory_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
        # 只有当用户没有明确指定batch_size时才进行GPU优化调整
        if args.batch_size is None:
            batch_size = base_batch_size * 2  # GPU可以处理更大批次
        else:
            batch_size = base_batch_size  # 使用用户指定的值
        epochs = base_epochs
        # 只有当用户没有明确指定learning_rate时才进行GPU优化调整
        if args.learning_rate is None:
            learning_rate = base_lr * 1.2  # 稍微提高学习率加速训练
        else:
            learning_rate = base_lr  # 使用用户指定的值
        
        print(f"GPU优化配置 (显存: {gpu_memory_gb:.1f}GB)")
    else:
        # CPU设备，使用合理参数
        batch_size = base_batch_size  # 使用用户指定的值或默认值
        epochs = base_epochs  # 使用用户指定的值或默认值
        learning_rate = base_lr  # 使用用户指定的值或默认值
