...
class KnowledgeDistillationLoss(nn.Module):
    ...
    def forward(self, 
        ...
        # 验证labels张量值范围，防止CUDA设备端断言错误
        num_classes = student_logits.size(1)  # 从学生模型输出推断类别数
        labels_min, labels_max = labels.min().item(), labels.max().item()
        if labels_min < 0 or labels_max >= num_classes:
            import logging
            logging.warning(f"KnowledgeDistillation: Labels张量值超出范围: min={labels_min}, max={labels_max}, num_classes={num_classes}")
            # 将labels值限制在有效范围内
        ...
        if torch.isnan(labels).any() or torch.isinf(labels).any():
            import logging
            logging.error("KnowledgeDistillation: Labels张量包含NaN或无穷值")
            # 将NaN和无穷值替换为0
...
class MultiTeacherDistillation(nn.Module):
    ...
    def compute_distillation_loss(self, x: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        计算蒸馏损失（用于nas_distillation.py调用）
        Args:
            x: 输入数据
            labels: 真实标签
        Returns:
...
if __name__ == "__main__":
    # 测试知识蒸馏模块
    from model import BasicModelBank  # 动态导入避免循环导入
    ...
    # 创建测试学生模型
    student = BasicModelBank(model_name='VNet3D', device=device).model
