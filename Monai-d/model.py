...
class SpecializedModelFactory:
    ...
    def _create_nas_distillation_model(self):
        ...
        nas_distillation = NASDistillationIntegration(
            ...
            # NAS相关参数
            nas_type=self.kwargs.get('nas_type', 'darts'),
            base_channels=self.kwargs.get('base_channels', 8),
            num_layers=self.kwargs.get('num_layers', 3),
            max_layers=self.kwargs.get('max_layers', 8),
            start_layers=self.kwargs.get('start_layers', 1),
            # 教师模型预训练参数
            teacher_pretrain_epochs=self.kwargs.get('teacher_pretrain_epochs', 50),
            teacher_learning_rate=self.kwargs.get('teacher_learning_rate', 1e-4),
            # 知识蒸馏参数映射
            distillation_temperature=self.kwargs.get('distillation_temperature'),
            distillation_alpha=self.kwargs.get('distillation_alpha'),
            # 其他参数
            auto_adjust=self.kwargs.get('auto_adjust', False)
        )
