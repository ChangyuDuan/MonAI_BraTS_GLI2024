...
# 4. 跳过教师模型预训练（使用随机初始化或已有预训练权重）
# 移除 --pretrain_teachers 和 --teacher_epochs 参数
python main.py --mode train --data_dir /path/to/BraTS_data --model_category advanced --model_type distillation --student_model UNet --epochs 250 --device cuda --output_dir ./outputs --pretrained_dir ./pretrained_teachers

# 5. 跳过预训练的最简化命令
python main.py --mode train --data_dir /path/to/BraTS_data --model_category advanced --model_type distillation
```
...
- `--pretrain_teachers`：启用教师模型预训练（可选）
  - **添加此参数**：程序会先预训练所有教师模型，然后进行知识蒸馏
  - **移除此参数**：跳过预训练步骤，直接使用随机初始化或已有预训练权重进行蒸馏
- `--teacher_epochs 100`：教师模型预训练轮数（默认100，仅在启用预训练时有效）
...
**`--pretrain_teachers` 参数的作用：**
- 这是一个 `action='store_true'` 类型的参数
- **当存在时**：程序会在知识蒸馏前先预训练所有教师模型
- **当不存在时**：程序会直接使用随机初始化的教师模型进行知识蒸馏

**不预训练的情况下：**
- **教师模型权重**：教师模型将使用随机初始化的权重
- **权重加载机制**：程序会尝试从 `--pretrained_dir` 目录加载已存在的预训练权重
- **异常处理**：如果找不到预训练权重，会显示警告信息并继续使用随机权重
- **训练流程**：知识蒸馏训练会直接开始，教师模型和学生模型同时训练
...
无需手动指定模型类型参数，系统会自动识别并加载相应的模型架构。
...
- **数据处理**: NumPy + NiBabel
- **可视化**: Matplotlib + TensorBoard（支持中文字体）
- **数据分析**: Pandas
- **进度显示**: tqdm

## 可视化中文字体支持

项目已完全解决matplotlib中文显示问题，支持在所有可视化图表中正常显示中文文字。

### 自动字体配置

项目会自动检测并配置系统可用的中文字体：

- **Windows系统**: 微软雅黑、黑体、宋体、楷体、仿宋
- **macOS系统**: 苹方、黑体-简、华文黑体
- **Linux系统**: 文泉驿微米黑、思源黑体、Noto Sans CJK
...
训练完成后，可以将复合架构模型部署到生产环境中进行实际应用。项目提供了完整的部署解决方案。
