...
class MultiTeacherDistillation(nn.Module):
    ...
    def forward(self, x: torch.Tensor, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            for teacher in self.teachers:
                teacher_output = teacher.model(x)
    ...
    def get_teacher_ensemble_prediction(self, x: torch.Tensor) -> torch.Tensor:
        """
        获取教师模型集成预测
        ...
        with torch.no_grad():
            for teacher in self.teachers:
                pred = teacher.model(x)
                teacher_predictions.append(pred)
        
        # 加权平均
        ensemble_pred = torch.zeros_like(teacher_predictions[0])
        for i, pred in enumerate(teacher_predictions):
            ensemble_pred += self.teacher_weights[i] * pred
