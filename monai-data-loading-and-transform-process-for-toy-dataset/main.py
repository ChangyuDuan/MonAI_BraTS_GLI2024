...
def run_simplified_training(args, device: torch.device):
    ...
    try:
        ...
        elif not (hasattr(args, 'model_category') and args.model_category == 'advanced'):
            # 单模型训练（基础模型）
...
def main():
    ...
    parser.add_argument(
        ...
        default='basic',
        help='模型类别：basic（基础模型）或 advanced（复合架构模型）'
    )
    ...
    parser.add_argument(
        ...
        default='single',
        help='复合架构模型类型：single（单模型）、fusion（融合网络）、distillation（知识蒸馏）、nas（神经架构搜索）'
    )
