...
class InferenceEngine:
    ...
    def _load_best_model(self) -> Union[BasicModelBank, AdvancedModelBank]:
        """加载最佳模型"""
        ...
        if 'config' in checkpoint and 'model_state_dict' in checkpoint:
            ...
            if model_category == 'advanced':
                # 高级模型
                try:
                    model_creator = ModelFactory.create_model(
                    ...
                    if hasattr(model_creator, 'model'):
                        ...
                        # 对于某些高级模型，可能需要特殊的加载方式
                        print(f"警告: 高级模型 {model_type} 可能需要特殊的状态加载方式")
                        
                    print(f"成功加载高级模型: {model_type}")
        ...
        elif hasattr(model_creator, 'eval'):
            model_creator.eval()
            ...
            if isinstance(self.model_creator, AdvancedModelBank):
                ...
                try:
                    if hasattr(self.model_creator, 'predict'):
                        # 使用模型自定义的预测方法
                        prediction = self.model_creator.predict(image_tensor)
                    elif hasattr(self.model_creator, 'model'):
                        # 使用滑动窗口推理
                        ...
                        # 直接调用模型
                        prediction = self.model_creator(image_tensor)
                        
                    print(f"高级模型推理完成，输出形状: {prediction.shape}")
                    ...
                    if hasattr(self.model_creator, 'model'):
                        ...
                        raise RuntimeError("无法进行推理：模型不支持标准推理接口")
            else:
                # 基础模型推理
                prediction = sliding_window_inference(
                    inputs=image_tensor,
