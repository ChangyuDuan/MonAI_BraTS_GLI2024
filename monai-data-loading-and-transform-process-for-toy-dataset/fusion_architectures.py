...
class FusionNetworkArchitecture(nn.Module):
    ...
    def __init__(self, 
        ...
        self.feature_adapters = nn.ModuleList([
            nn.ModuleList([
                nn.Conv3d(ch, ch, 1) for ch in fusion_channels
    ...
    def extract_multi_level_features(self, x: torch.Tensor) -> List[List[torch.Tensor]]:
        ...
        num_levels = max(len(features) for features in all_features)
        level_features = []
        ...
        for level in range(num_levels):
            ...
            for model_idx in range(len(all_features)):
                if level < len(all_features[model_idx]):
                    ...
                    # 适配特征维度
                    if level < len(self.feature_adapters[model_idx]):
                        feature = self.feature_adapters[model_idx][level](feature)
