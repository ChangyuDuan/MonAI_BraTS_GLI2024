...
def run_simplified_training(args, device: torch.device):
    ...
    if args.enable_ensemble and len(args.model_names) > 1:
        print(f"检测到多模型集成训练模式 - 并行训练{len(args.model_names)}个模型")
    ...
    try:
        ...
        if len(args.model_names) > 1:
            # 多模型训练（自动集成）
            if args.parallel:
                print(f"\n启动多模型并行训练模式（自动集成）- 同时训练{len(args.model_names)}个模型")
            else:
                print(f"\n启动多模型逐个训练模式（自动集成）- 依次训练{len(args.model_names)}个模型")
        elif args.enable_ensemble:
            # 启用完整集成（7个模型）
            if args.parallel:
                print(f"\n启动完整集成并行训练模式 - 同时训练{len(args.model_names)}个模型")
            else:
                print(f"\n启动完整集成逐个训练模式 - 依次训练{len(args.model_names)}个模型")
        else:
            # 单模型训练
        ...
        if len(trained_models) > 1:
            print(f"\n创建集成模型，融合{len(trained_models)}个训练好的模型")
            
            # 创建集成模型配置
            ensemble_config = merge_args_with_config(args, device)
            ensemble_config.update({
                'trained_models': trained_models,
                'output_dir': f'{args.output_dir}/ensemble_model'
            })
            
            # 创建集成模型
            ensemble_trainer = ModelTrainer(ensemble_config)
            ensemble_trainer.create_ensemble_model()
            
            print(f"[成功] 集成模型创建完成 - 通过平均预测和投票机制融合多个模型")
        
        # 额外的完整集成（当显式启用ensemble且训练了所有7个模型时）
        if args.enable_ensemble and len(trained_models) == 7:
            print(f"\n创建额外的完整集成模型（7个模型）")
            ...
            # 创建完整集成模型
            full_ensemble = create_full_ensemble(device=str(device))
            
            # 保存完整集成模型信息
            full_ensemble_path = f'{args.output_dir}/full_ensemble_model.pth'
            os.makedirs(os.path.dirname(full_ensemble_path), exist_ok=True)
            ...
            print(f"[成功] 完整集成模型创建完成，包含所有7个模型架构")
        
        print(f"\n[成功] 简化训练流程完成！")
        print(f"训练了 {len(trained_models)} 个模型")
        if len(trained_models) > 1:
            print(f"已创建集成模型 - 将{len(trained_models)}个模型融合为一个新的集成架构")
        if args.enable_ensemble and len(trained_models) > 1:
            print(f"已创建额外的完整集成模型 - 包含所有7个模型架构的融合")
...
def main():
    ...
    # 处理集成模式逻辑
    # 如果没有指定模型且没有启用ensemble，默认训练单个UNet模型
    if not args.model_names and not args.enable_ensemble:
        args.model_names = ['UNet']  # 默认单模型训练
    
    # 多模型训练时自动启用集成（除非用户明确指定了model_names但没有启用ensemble）
    if len(args.model_names) > 1:
        # 多模型训练时，集成是自动启用的，不需要额外设置
        pass
    elif args.enable_ensemble:
        # 如果启用了ensemble但没有指定模型，使用所有7个模型
