...
class BraTSTrainer:
    ...
    def _setup_model(self):
        ...
        设置模型、损失函数和指标
        # 后处理变换
        self.post_pred = Compose([AsDiscrete(argmax=True, to_onehot=5)])
        self.post_label = Compose([AsDiscrete(to_onehot=5)])        统一使用自适应损失函数策略和完整评估指标
        """
        print("设置模型...")
        print("[策略] 所有模型类型统一使用:")
        ...
        if use_ensemble:
            ...
            temp_model_creator = SimpleBraTSModel(device=self.device)
        ...
        # 验证损失函数和指标配置
        loss_info = temp_model_creator.get_loss_info() if use_ensemble else self.model_creator.get_loss_info()
