...
from model import SimpleBraTSModel, SimpleEnsembleModel, get_all_supported_models
...
class BraTSEvaluator:
    ...
    def _load_model(self):
        ...
        if use_ensemble:
            ...
            # 创建集成模型（统一使用自适应损失函数策略）
            models = []
            for model_name in ensemble_models:
                model_creator = SimpleBraTSModel(
            ...
            print(f"加载单个模型: {model_name} (SimpleBraTSModel)")
            self.model_creator = SimpleBraTSModel(
                model_name=model_name,
                device=self.device,
                use_adaptive_loss=True,  # 强制使用自适应损失函数
                use_full_metrics=True    # 强制使用完整评估指标
        ...
        if self.is_ensemble:
            ...
            if 'ensemble_state_dicts' in checkpoint:
                # 如果保存了集成模型的所有权重
                ensemble_weights = checkpoint['ensemble_state_dicts']
                for i, model_creator in enumerate(self.ensemble_model.models):
                    if i < len(ensemble_weights):
                        model_creator.get_model().load_state_dict(ensemble_weights[i])
                        model_creator.get_model().eval()
                print(f"成功加载集成模型权重，包含 {len(ensemble_weights)} 个子模型")
            else:
                # 如果只有单个模型权重，为所有子模型加载相同权重
                print("警告: 检查点中未找到集成模型权重，将为所有子模型加载相同的权重")
                ...
                for model_creator in self.ensemble_model.models:
                    try:
                        model_creator.get_model().load_state_dict(state_dict)
                        model_creator.get_model().eval()
                    except Exception as e:
                        print(f"警告: 无法为模型 {model_creator.model_name} 加载权重: {e}")
        else:
            # 单个模型权重加载
            if 'model_state_dict' in checkpoint:
        ...
        if self.is_ensemble:
            ...
            total_size = 0
            for model_creator in self.ensemble_model.models:
                model_info = calculate_model_size(model_creator.get_model())
                total_params += model_info['total_params']
                total_size += model_info['model_size_mb']
            
            print(f"模型类型: 集成模型 ({len(self.ensemble_model.models)} 个子模型)")
            print(f"子模型列表: {[m.model_name for m in self.ensemble_model.models]}")
            print(f"集成模式: {use_ensemble}")
